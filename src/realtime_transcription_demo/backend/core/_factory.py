...
try:
    ...
    # Source-only deployments may not include generated _metadata.py.
    app_name = "realtime-transcription-demo"
    api_prefix = "/api"
    dist_dir = Path(__file__).resolve().parents[2] / "__dist__"
    ...
    *,
    ...
    for dep in LifespanDependency._registry:
        try:
            ...
            logger.error("Failed to instantiate dependency %s: %s", dep.__name__, e)
            raise
